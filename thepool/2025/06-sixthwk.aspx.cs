...
public partial class _2025_06_sixthwk : Page
{
    ...
    public string ScheduleUrl { get; } = $"{CredentialStore.ApiBaseUrl}/{SeasonSegment}/full_game_schedule.json?date=from-20251009-to-20251013";
    ...
    private static readonly int[] ScoreboardSearchOffsets = { -1, 1, -2, 2 };
    private static readonly string[] EasternTimeZoneIds = { "Eastern Standard Time", "America/New_York", "US/Eastern" };
    ...
    private void BuildDayGroups(WebClient client)
    {
        ...
        PreloadScoreboards(client, games);
        AttachScores(client, games);
        BuildGroupsFromGames(games);

        _orderedGames.AddRange(games);
        NumberOfGames = _orderedGames.Count;
    }

    private List<GameDisplay> CreateOrderedGames()
    {
        ...
        return Schedule.fullgameschedule.gameentry
            .Select((entry, index) => new GameDisplay(entry, index)
            {
                DisplayDate = ParseDate(entry.date, entry.time) ?? DateTime.MinValue,
                StartTime = ParseGameTime(entry.time)
            })
    ...
    private void PreloadScoreboards(WebClient client, IEnumerable<GameDisplay> games)
    {
        foreach (var date in games
            .Select(g => g.DisplayDate)
            .Where(IsValidDate)
            .Select(d => d.Date)
            .Distinct())
        {
    ...
    private void AttachScores(WebClient client, List<GameDisplay> games)
    {
        foreach (var game in games)
        {
            if (TryAttachScore(game))
            {
                continue;
            }
    ...
    private void BuildGroupsFromGames(IEnumerable<GameDisplay> games)
    {
    ...
    private bool TryAttachScore(GameDisplay game)
    {
        if (_scoresByGameId.TryGetValue(game.Schedule.id, out var score) && score != null)
        {
    ...
    private static int CompareGames(GameDisplay left, GameDisplay right)
    {
        ...
        int dateComparison = left.DisplayDate.CompareTo(right.DisplayDate);
        if (dateComparison != 0)
        {
            return dateComparison;
        }

        var leftTime = left.StartTime ?? TimeSpan.MaxValue;
        var rightTime = right.StartTime ?? TimeSpan.MaxValue;
    ...
    private IReadOnlyList<Gamescore> CacheScores(WebClient client, DateTime date)
    {
        ...
        {
            _scoresByDate[normalized] = Array.Empty<Gamescore>();
    ...
    private static DateTime ConvertToEasternDate(DateTimeOffset dto)
    {
        ...
        {
            return TimeZoneInfo.ConvertTime(dto, EasternTimeZone).DateTime;
        }
    ...
    {
        foreach (var id in EasternTimeZoneIds)
        {
            try
            {
