...
public partial class _2020_05_5wk : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        OleDbCommand comPicks = new OleDbCommand("SELECT * FROM five2020 ORDER BY uCase(lastname),uCase(firstname)", connPicks);
    ...
    public int ScoringGames(string scoringFirstName, string scoringLastName, string game1, string game2, string game3, string game4, string game5, string game6, string game7, string game8, string game9, string game10, string game11, string game12, string game13, string game14, string game15)
    {
        ...
        string[] tuesdaypicks = { game15 };

        for (int i = 0; i < livescores4.scoreboard.gameScore.Length; i++)
        {
            if (tuesdaypicks[i] == (livescores4.scoreboard.gameScore[i].game.awayTeam.City + " " + livescores4.scoreboard.gameScore[i].game.awayTeam.Name))
            {
                int x;
                int y;
                Int32.TryParse(livescores4.scoreboard.gameScore[i].awayScore, out x);
                Int32.TryParse(livescores4.scoreboard.gameScore[i].homeScore, out y);
                if (x > y)
                {
                    score++;
                }
