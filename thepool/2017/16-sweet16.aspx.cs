...
public partial class _2017_16_sweet16 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        OleDbCommand comPicks = new OleDbCommand("SELECT * FROM sixteen2017 ORDER BY uCase(lastname),uCase(firstname)", connPicks);
    ...
    public int ScoringGames(string scoringFirstName, string scoringLastName, string game1, string game2, string game3, string game4, string game5, string game6, string game7, string game8, string game9, string game10, string game11, string game12, string game13, string game14, string game15, string game16)
    {
        ...
        string[] satPicks = { game1, game2 };

        for (int i = 0; i < livescoressat.scoreboard.gameScore.Length; i++)
        {
            if (satPicks[i] == (livescoressat.scoreboard.gameScore[i].game.awayTeam.City + " " + livescoressat.scoreboard.gameScore[i].game.awayTeam.Name))
            {
                int x;
                int y;
                Int32.TryParse(livescoressat.scoreboard.gameScore[i].awayScore, out x);
                Int32.TryParse(livescoressat.scoreboard.gameScore[i].homeScore, out y);
                if (x > y)
                {
                    score++;
                }
        ...
        string[] mondaypicks = { game15, game16 };
