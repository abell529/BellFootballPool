...
public partial class _2017_12_thanksgivingweek : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        OleDbCommand comPicks = new OleDbCommand("SELECT * FROM twelve2017 ORDER BY uCase(lastname),uCase(firstname)", connPicks);
    ...
    {
        int score = 0;
        string[] thanksPicks = { game1, game2, game3 };

        for (int i = 0; i < livescores.scoreboard.gameScore.Length; i++)
        {
            if (thanksPicks[i] == (livescores.scoreboard.gameScore[i].game.awayTeam.City + " " + livescores.scoreboard.gameScore[i].game.awayTeam.Name))
            {
                int x;
                int y;
                Int32.TryParse(livescores.scoreboard.gameScore[i].awayScore, out x);
                Int32.TryParse(livescores.scoreboard.gameScore[i].homeScore, out y);
                if (x > y)
                {
                    score++;
                }
