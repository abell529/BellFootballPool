...
public partial class _2017_07_seven : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        OleDbCommand command = new OleDbCommand("SELECT * from 2017", connection);
        ...
        OleDbCommand comPicks = new OleDbCommand("SELECT * FROM seven2017 ORDER BY uCase(lastname),uCase(firstname)", connPicks);
    ...
    public int ScoringGames(string scoringFirstName, string scoringLastName, string game1, string game2, string game3, string game4, string game5, string game6, string game7, string game8, string game9, string game10, string game11, string game12, string game13, string game14, string game15)
    {
        ...
        string[] gamePicks = { game2, game3, game4, game5, game6, game7, game8, game9, game10, game11, game12, game13, game14 };

        for (int i = 0; i < livescores2.scoreboard.gameScore.Length; i++)
        {
            if (gamePicks[i] == (livescores2.scoreboard.gameScore[i].game.awayTeam.City + " " + livescores2.scoreboard.gameScore[i].game.awayTeam.Name))
            {
                int x;
                int y;
                Int32.TryParse(livescores2.scoreboard.gameScore[i].awayScore, out x);
                Int32.TryParse(livescores2.scoreboard.gameScore[i].homeScore, out y);
                if (x > y)
                {
                    score++;
                }
