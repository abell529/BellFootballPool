...
public partial class _2021_15_weekbeforexmas : System.Web.UI.Page
{
    ...
    public int numberofgamescorestue;
    ...
    public LiveScoring livescorestue;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var responsescorestue = client.DownloadString(urlscoreswk1tue);
        ...
        livescorestue = JsonConvert.DeserializeObject<LiveScoring>(responsescorestue);
        ...
        numberofgamescorestue = livescorestue.scoreboard.gameScore.Length;
        ...
        OleDbCommand command = new OleDbCommand("SELECT * from 2021", connection);
        ...
        OleDbCommand comPicks = new OleDbCommand("SELECT * FROM fifteen2021 ORDER BY uCase(lastname),uCase(firstname)", connPicks);
    ...
    public int ScoringGames(string scoringFirstName, string scoringLastName, string game1, string game2, string game3, string game4, string game5, string game6, string game7, string game8, string game9, string game10, string game11, string game12, string game13, string game14, string game15, string game16)
    {
        ...
        string[] gamePicks = { game3, game4, game5, game6, game7, game8, game9, game10, game11, game12 };
        ...
        for (int a = 0; a < mondaypicks.Length; a++)
        {
            int b = 0;
            while (b < livescores3.scoreboard.gameScore.Length)
            {

                if (mondaypicks[a] == (livescores3.scoreboard.gameScore[b].game.awayTeam.City + " " + livescores3.scoreboard.gameScore[b].game.awayTeam.Name))
                {
                    int x;
                    int y;
                    Int32.TryParse(livescores3.scoreboard.gameScore[b].awayScore, out x);
                    Int32.TryParse(livescores3.scoreboard.gameScore[b].homeScore, out y);
                    if (x > y)
                    {
                        score++;
                    }
                ...
                else if (mondaypicks[a] == (livescores3.scoreboard.gameScore[b].game.homeTeam.City + " " + livescores3.scoreboard.gameScore[b].game.homeTeam.Name))
                {
                    int x;
                    int y;
                    Int32.TryParse(livescores3.scoreboard.gameScore[b].awayScore, out x);
        ...
        string[] tuesdaypicks = { game15, game16 };

        for (int a = 0; a < tuesdaypicks.Length; a++)
        {
            int b = 0;
            while (b < livescorestue.scoreboard.gameScore.Length)
            {

                if (tuesdaypicks[a] == (livescorestue.scoreboard.gameScore[b].game.awayTeam.City + " " + livescorestue.scoreboard.gameScore[b].game.awayTeam.Name))
                {
                    int x;
                    int y;
                    Int32.TryParse(livescorestue.scoreboard.gameScore[b].awayScore, out x);
                    Int32.TryParse(livescorestue.scoreboard.gameScore[b].homeScore, out y);
                    if (x > y)
                    {
                        score++;
                    }
                ...
                else if (tuesdaypicks[a] == (livescorestue.scoreboard.gameScore[b].game.homeTeam.City + " " + livescorestue.scoreboard.gameScore[b].game.homeTeam.Name))
                {
                    int x;
                    int y;
                    Int32.TryParse(livescorestue.scoreboard.gameScore[b].awayScore, out x);
