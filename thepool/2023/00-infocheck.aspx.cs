...
public partial class _2020_00_infocheck : System.Web.UI.Page
{
    ...
    public int numberofgamescores4;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var responsescores = client.DownloadString(urlscores);
        // var responsescores2 = client.DownloadString(urlscoreswk1pt2);
        // var responsescores3 = client.DownloadString(urlscoreswk1pt3);
        // var responsescores4 = client.DownloadString(string.Concat(urlscoreswk1pt1, urlscoreswk1pt2, urlscoreswk1pt3));
        ...
        livescores = JsonConvert.DeserializeObject<Root>(responsescores);
        ...
        // livescores4 = JsonConvert.DeserializeObject<LiveScoring>(responsescores4);
        ...
        OleDbCommand comPicks = new OleDbCommand("SELECT * FROM four2020 ORDER BY uCase(lastname),uCase(firstname)", connPicks);
        ...
        while (readerPicks.Read())
        {
            ...
            correctPicks.Add(ScoringGames(readerPicks["firstname"].ToString(), readerPicks["lastname"].ToString(), readerPicks["game1"].ToString(), readerPicks["game2"].ToString(), readerPicks["game3"].ToString(), readerPicks["game4"].ToString(), readerPicks["game5"].ToString(), readerPicks["game6"].ToString(), readerPicks["game7"].ToString(), readerPicks["game8"].ToString(), readerPicks["game9"].ToString(), readerPicks["game10"].ToString(), readerPicks["game11"].ToString(), readerPicks["game12"].ToString(), readerPicks["game13"].ToString(), readerPicks["game14"].ToString(), readerPicks["game15"].ToString()));
    ...
    public int ScoringGames(string scoringFirstName, string scoringLastName, string game1, string game2, string game3, string game4, string game5, string game6, string game7, string game8, string game9, string game10, string game11, string game12, string game13, string game14, string game15)
    {
        int score = 0;
        string[] gamePicks = { game1, game2, game3, game4, game5, game6, game7, game8, game9, game10, game11, game12, game13, game14, game15 };

        for (int a = 0; a < gamePicks.Length; a++)
        {
            int b = 0;
            while (b < livescores.games.Count)
            {

                if (gamePicks[a] == (livescores.references.teamReferences[b].city + " " + livescores.references.teamReferences[b].name))
                {
                    if (livescores.games[b].score.awayScoreTotal > livescores.games[b].score.homeScoreTotal)
                    {
                        score++;
                    }
                    b++;
                }
                else if (gamePicks[a] == (livescores.scoreboard.gameScore[b].game.homeTeam.City + " " + livescores.scoreboard.gameScore[b].game.homeTeam.Name))
                {
                    int x;
                    int y;
                    Int32.TryParse(livescores.scoreboard.gameScore[b].awayScore, out x);
                    Int32.TryParse(livescores.scoreboard.gameScore[b].homeScore, out y);
                    if (x < y)
                    {
                        score++;
                    }
